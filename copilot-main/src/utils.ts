...
declare global {
  function MernifierAPIBrowserOnProgress(
...
/**
 * This function is injected into the Mernifier webapp page using puppeteer. It
...
export async function browserPostEventStream(
  ...
  try {
    ...
    if (!res.ok) {
      return {
        error: {
          message: `MernifierAPI error ${res.status || res.statusText}`,
        ...
        async function onMessage(data: string) {
          ...
          try {
            ...
            if (partialResponse) {
              ...
              if (window.MernifierAPIBrowserOnProgress) {
                ...
                await window.MernifierAPIBrowserOnProgress(partialChatResponse);
              }
            }
          } catch (err) {
    ...
    if (timeoutMs) {
      ...
      return await pTimeout(responseP, {
        ...
        message: "Mernifier timed out waiting for response",
      });
    } else {
