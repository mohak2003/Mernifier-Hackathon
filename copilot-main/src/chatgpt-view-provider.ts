...
const logger = vscode.window.createOutputChannel("Mernifier Copilot");
...
export default class ChatGptViewProvider implements vscode.WebviewViewProvider {
  ...
  private get systemContext() {
    return `You are Mernifier helping the User with coding.
  ...
  public async sendApiRequest(
    ...
    // If the Mernifier view is not in focus/visible; focus on it to render Q&A
    ...
    try {
      ...
      if (hasContinuation) {
        ...
        vscode.window
          .showInformationMessage(
            "It looks like Mernifier didn't complete their answer for your coding question. You can ask it to continue and combine the answers.",
      ...
      if (this.subscribeToResponse) {
        vscode.window
          .showInformationMessage(
            "Mernifier responded to your question.",
            "Open conversation",
          )
          .then(async () => {
      ...
      if (error?.response?.status || error?.response?.statusText) {
        ...
        vscode.window
          .showErrorMessage(
            "An error occured. If this is due to max_token you could try `Mernifier: Clear Conversation` command and retry sending your prompt.",
        ...
        message =
          "Make sure you are properly signed in. If you are using Browser Auto-login method, make sure the browser is open (You could refresh the browser tab manually if you face any issues, too). If you stored your API key in settings.json, make sure it is accurate. If you stored API key in session, you can reset it with `Mernifier: Reset session` command. (HTTP 401 Unauthorized) Potential reasons: \r\n- 1.Invalid Authentication\r\n- 2.Incorrect API key provided.\r\n- 3.Incorrect Organization provided. \r\n See https://platform.openai.com/docs/guides/error-codes for more details.";
      } else if (error.statusCode === 403) {
        message =
          "Your token has expired. Please try authenticating again. (HTTP 403 Forbidden)";
      } else if (error.statusCode === 404) {
        message = `Your model: '${this.model}' may be incompatible or you may have exhausted your Mernifier subscription allowance. (HTTP 404 Not Found)`;
